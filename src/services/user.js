...
export default class {
	...
	static updateSmsEdgeKey = async (token, data) => {
		let result = { data: null, error: null };
		const headers = {};
		headers[Constants.TOKEN_NAME] = token;

		await axios
			.post(`${process.env.REACT_APP_API_URL}/users/update-key`, data, {
				headers: headers
			})
			.then(resp => {
				if (resp.status === 200) {
	...
	static updateSearchKeywords = async (token, data) => {
		let result = { data: null, error: null };
		const headers = {};
		headers[Constants.TOKEN_NAME] = token;
		...
		await axios
			.post(
				`${process.env.REACT_APP_API_URL}/users/update-keywords`,
				data,
				{
					headers: headers
				}
