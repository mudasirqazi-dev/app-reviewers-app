...
function Home() {
  ...
  const [selectedPhoneNumbersForSms, setSelectedPhoneNumbersForSms] = useState(
    []
  );
  ...
  const handleSearch = () => {
    ...
    if (keyword?.length < 1) {
      setInfoMessage(`Please provide atleast one application name.`);
      return;
    }
    setList([]);
    setList1([]);
    ...
    appService
      ...
      .then((result) => {
        ...
        let t = [];
        result.data.history.forEach((h) => h.results.forEach((k) => t.push(k)));
        setList1(t);
        list = list.filter(
          (k) =>
            t.findIndex(
          ...
          .then((result) => {
            ...
            list = list.filter(
              (k) =>
                t.findIndex(
                  (l) =>
                    l?.username?.toLowerCase() === k.username?.toLowerCase()
                ) === -1
            );
            setList(list);
            setHistory(t);
  ...
  return (
    <>
      <Confirm
        ...
        body={
          <Typography>
            Charges for more user details are {settings?.cost} credits per phone
            ...
            records ({phoneNumbersCount} phone numbers) and it will cost{" "}
            <Typography
              color="primary"
              variant="span"
              sx={{ fontWeight: "bold" }}
        ...
        <Grid item xs={12} md={6}>
          <Box component={Paper} sx={{ p: 2 }}>
            ...
            <small style={{ color: "#BCBCBC" }}>
              * Type app name and press Enter button <br />* Double click to
              edit a chip
            </small>
            <Button
              text="Search"
                ...
                <Button
                  ...
                  onClick={() => {
                    checkedRows.length > 0
                      ...
                      : setErrorMessage("Please select the names first.");
                  }}
                  color="info"
                            ...
                            <span>
                              ...
                              {row?.contacts > 1 && (
                                <>
                                  ...
                                  >
                                    {row?.contacts - 1} more phone numbers with
                                    similar name
                                  </div>
                                </>
                              )}
                      ...
                      {(rowsPerPage1 > 0
                        ...
                        >
                          <TableCell component="td" scope="row" align="left">
                            ...
                            <div style={{ fontSize: 11 }}>({row?.app})</div>
                          </TableCell>
                          <TableCell component="td" scope="row" align="left">
                            {row?.country}
                          </TableCell>
